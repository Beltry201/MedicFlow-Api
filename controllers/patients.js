...
export const getPatientDetails = async (req, res) => {
  try {
    ...
    console.error(error);
    res.status(500).json({ success: false, message: 'Failed to get patient details', error: error.message });
  }
};
...
export const deletePatient = async (req, res) => {
  try {
    ...
    res.status(200).json({ success: true, message: 'Patient deactivated successfully' });
  } catch (error) {
    console.error(error);
    res.status(500).json({ success: false, message: 'Failed to deactivate patient', error: error.message });
  }
};
...
export const getDoctorPatients = async (req, res) => {
    try {
      ...
      console.error(error);
      res.status(500).json({ success: false, message: 'Failed to get doctor patients', error: error.message });
    }
  };
